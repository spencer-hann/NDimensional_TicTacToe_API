...
class ReinforcementAgent:
    ...
    def __init__(self, marker=b'O', n=10000, m=20000, eta=0.01, g=0.9, e=1):
        """
        Set initial values.
    ...
    def _act(self, game):
        ...
        for _ in range(self._m):
            ...
            if not (game.game_over or game.is_full()):
                opponent_winner = game.take_turn(self._opponent.next_move(game))
            if winner == self.marker:
                reward = 20
            elif opponent_winner == self._opponent.marker:
                reward = -20
            elif winner == 'C':  # Tie
                reward = 0
            else:
    ...
    def _train(self, size, dim):
        ...
        rewards = []
        print(f'Training {self.name} Agent against a '
              f'{self._opponent.name} Agent')
