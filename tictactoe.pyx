...
cdef class Game:
    ...
    def __cinit__(Game self, int size=3, int dim=2):# default to 3x3 board
...
#            column/diagonal """
        cdef int[::1] init = np.zeros(self.dim, dtype=np.intc)
        cdef int[::1] terminal = np.empty(self.dim, dtype=np.intc)
